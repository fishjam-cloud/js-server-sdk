...
import { CredentialsApi, ResponseError } from '@fishjam-cloud/fishjam-openapi';
...
const responseError = (status: number, detail = 'error') =>
  new ResponseError(new Response(JSON.stringify({ detail }), { status }));
...
describe('FishjamClient live credential checks (mocked)', () => {
  ...
  it('FishjamClient.create rejects with InvalidFishjamCredentialsException on 404', async () => {
    validateSpy.mockRejectedValueOnce(responseError(404, 'Fishjam not found'));
  ...
  it('FishjamClient.create resolves and pings backend once on success', async () => {
    validateSpy.mockResolvedValueOnce(undefined as never);
