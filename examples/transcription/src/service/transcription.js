...
export class TranscriptionService {
    ...
    initFishjam() {
        const notifier = new FishjamWSNotifier(this.fishjamConfig, (error) => console.error('Fishjam websocket error: %O', error), (code, reason) => console.log(`Fishjam websocket closed. code: ${code}, reason: ${reason}`));
    ...
    async handlePeerConnected(message) {
        ...
        const agent = new FishjamAgent(this.fishjamConfig, agentToken, (error) => console.error('Fishjam agent websocket error: %O', error), (code, reason) => console.log(`Fishjam agent websocket closed. code: ${code}, reason: ${reason}`));
        ...
        const session = await this.ai.live.connect({
            ...
            callbacks: {
                ...
                onclose: (e) => console.log(`Connection to Gemini Live API for peer ${peerId} closed. code: ${e.code}, reason: ${e.reason}`),
    ...
    handleTranscription(peerId, msg) {
        ...
        if (transcription)
            console.log(`Peer ${peerId} said: "${transcription}".`);
    }
}
